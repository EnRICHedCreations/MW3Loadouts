...
const WEAPON_CLASS_COLORS: Record<string, string> = {
  Assault: "#7ab327", SMG: "#d4691e", Sniper: "#c8a228", LMG: "#b83232",
...
export function LoadoutGrid({ loadouts: initial }: { loadouts: Loadout[] }) {
  ...
  const [likedIds, setLikedIds] = useState<Set<string>>(() => {
    ...
    try { return new Set<string>(JSON.parse(localStorage.getItem("liked_loadouts") || "[]")); }
  ...
  const handleLike = async (e: React.MouseEvent, id: string) => {
    ...
    setLoadouts((prev) => prev.map((l) => l.id === id ? { ...l, likes: (l.likes ?? 0) + (alreadyLiked ? -1 : 1) } : l));
    const next = new Set(likedIds);
    alreadyLiked ? next.delete(id) : next.add(id);
    setLikedIds(next);
    localStorage.setItem("liked_loadouts", JSON.stringify(Array.from(next)));
    await supabase.rpc(alreadyLiked ? "decrement_likes" : "increment_likes", { loadout_id: id });
  };
  ...
  const filtered = useMemo(() => {
    ...
    if (search.trim()) {
      ...
      result = result.filter((l) =>
        l.title.toLowerCase().includes(q) || l.author.toLowerCase().includes(q) ||
        l.attachments.toLowerCase().includes(q) || l.weapon_class.toLowerCase().includes(q)
      );
    }
    switch (sort) {
    ...
    <>
      <div className={styles.controlBar}>
        <div className={styles.searchWrap}>
          ...
          <input className={styles.searchInput} type="text" placeholder="SEARCH LOADOUTS, AUTHORS, ATTACHMENTS..." value={search} onChange={(e) => setSearch(e.target.value)} />
          {search && <button className={styles.searchClear} onClick={() => setSearch("")}>✕</button>}
        </div>
        <div className={styles.sortWrap}>
          <span className={styles.sortLabel}>SORT:</span>
          {(["newest","oldest","most_liked","most_viewed"] as SortOption[]).map((s) => (
            <button key={s} className={`${styles.sortBtn} ${sort === s ? styles.sortActive : ""}`} onClick={() => setSort(s)}>
              {s === "newest" && "NEWEST"}{s === "oldest" && "OLDEST"}{s === "most_liked" && "❤ LIKED"}{s === "most_viewed" && "◉ VIEWED"}
            </button>
          ))}
        </div>
      </div>
      ...
      <div className={styles.filterBar}>
        ...
        <button className={`${styles.filterBtn} ${activeClass === null ? styles.filterActive : ""}`} onClick={() => setActiveClass(null)}>
        ...
        {WEAPON_CLASSES.filter((c) => loadouts.some((l) => l.weapon_class === c)).map((cls) => (
          <button key={cls} className={`${styles.filterBtn} ${activeClass === cls ? styles.filterActive : ""}`}
            ...
            {cls} <span className={styles.filterCount}>{loadouts.filter((l) => l.weapon_class === cls).length}</span>
          </button>
        ))}
      </div>
      ...
      {filtered.length === 0 ? (
        <div className={styles.emptyState}><p className={styles.emptyTitle}>NO RESULTS</p><p className={styles.emptyText}>Try a different search or filter.</p></div>
      ) : (
            ...
            <Link key={loadout.id} href={`/loadout/${loadout.id}`} className={styles.card}>
              ...
              <div className={styles.cardBody}>
                ...
                <div className={styles.cardMeta}>
                  <Link
                    href={`/profile/${encodeURIComponent(loadout.author)}`}
                  ...
                  <div className={styles.cardStats}>
                    <button className={`${styles.likeBtn} ${likedIds.has(loadout.id) ? styles.likeBtnActive : ""}`} onClick={(e) => handleLike(e, loadout.id)}>
                      ❤ {loadout.likes ?? 0}
                    </button>
